...
export const handlers = [
  ...
  rest.get("/mismatched-header-error", (req, res, ctx) => {
    return res(
      ctx.text("oh hello there"),
      ctx.set("Content-Type", "application/hal+banana")
    );
  }),
  ...
  rest.put("/increment", (req, res, ctx) => {
    const { amount } = req.body;
    count = count += amount;

    return res(ctx.json({ count }));
  }),
  rest.put("/decrement", (req, res, ctx) => {
    const { amount } = req.body;
    count = count -= amount;
  ...
  rest.put("/stop", (req, res, ctx) => {
    ...
    return res(
      ctx.json({
        success: true,
        message: "Intervals stopped. No more shennanigans"
      })
    );
  }),
  rest.put("/:id/increment", (req, res, ctx) => {
    const { amount } = req.body;
    const { id } = req.params;
  ...
  rest.put("/:id/decrement", (req, res, ctx) => {
    const { amount } = req.body;
    const { id } = req.params;
